...
| 40 | `C2S_ROOM_CREATE` | 客户端→服务器 | `C2SRoomCreate`（空） |
...
- **传入参数**：空消息 `C2SRoomCreate`。

**返回消息**

- 成功：推 `S2C_ROOM_STATE`，从字段 `room_id` 读 6 位房号。
...
- 自己回合：服务器帮你打出刚摸的那张（没有则打手牌最后一张）
- 吃碰杠胡阶段：服务器帮你过
- 断线：该座位立刻进入托管，按上面规则自动打
